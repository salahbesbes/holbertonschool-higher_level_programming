...
class TestBase(unittest.TestCase):
    ...
    def test_update_and_to_dictionary(self):
        # update *args
        b0 = Square(10)
        self.assertEqual(b0.to_dictionary(), {'id': 1, 'size': 10, 'x': 0, 'y': 0})
        b0.update(5, 5, 5, 5)
        self.assertEqual(b0.to_dictionary(), {'id': 5, 'size': 5, 'x': 5, 'y': 5})

        b1 = Square(100, 55)
        self.assertEqual(b1.to_dictionary(), {'id': 2, 'size': 100, 'x': 55, 'y': 0})
        b1.update(20)
        self.assertEqual(b1.to_dictionary(), {'id': 20, 'size': 100, 'x': 55, 'y': 0})

        b2 = Square(88, 23, 66)
        self.assertEqual(b2.to_dictionary(), {'id': 3, 'size': 88, 'x': 23, 'y': 66})
        b2.update(17, 44)
        self.assertEqual(b2.to_dictionary(), {'id': 17, 'size': 44, 'x': 23, 'y': 66})

        b3 = Square(28, 33, 75, 88)
        self.assertEqual(b3.to_dictionary(), {'id': 88, 'size': 28, 'x': 33, 'y': 75})
        b3.update(5, 4, 3, 2)
        self.assertEqual(b3.to_dictionary(), {'id': 5, 'size': 4, 'x': 3, 'y': 2})

        # update *kwargs
        b6 = Square(5, 5)
        b6.update(id=66, size=77, x=8, y=99)
        self.assertEqual(b6.to_dictionary(), {'id': 66, 'size': 77, 'x': 8, 'y': 99})

        b7 = Square(5, 5)
        b7.update(id=66, size=77, x=8, y=99, garbage="test")
        with self.assertRaises(AttributeError):
            self.assertEqual(b7.garbage, "test")
        self.assertEqual(b7.to_dictionary(), {'id': 66, 'size': 77, 'x': 8, 'y': 99})
